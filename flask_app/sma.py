...
# Title
st.title("Nifty 500 Stock Screener with Risk-Reward Analysis")
...
max_symbols = st.sidebar.number_input("Max Symbols to Screen (for performance)", min_value=1, max_value=500, value=500)
...
def screen_stocks(symbols, filter_param='Volume', threshold=0, short_window=9, long_window=21, vcp_threshold=0.1, signal_filter='All', summary_type='Detailed'):
    data = {}
    summary_data = []

    for symbol in symbols:
        try:
            stock_data = fetch_data(symbol, period="6mo", interval="1d")
            if stock_data.empty:
            ...
            has_vcp = detect_vcp_pattern(stock_data, vcp_threshold)
            ...
            # Risk-Reward Metrics
            historical_trades = analyze_trades(stock_data, short_window, long_window)
            win_rate = historical_trades['Win'].mean() * 100 if not historical_trades.empty else 0
            avg_win = historical_trades[historical_trades['Win'] == 1]['Profit'].mean() if not historical_trades.empty else 0
            avg_loss = historical_trades[historical_trades['Win'] == 0]['Profit'].mean() if not historical_trades.empty else 0
            ...
            if summary_type == 'Detailed':
                latest_summary = {
                    ...
                    'SMA9': latest_data['SMA_short'],
                    'SMA21': latest_data['SMA_long'],
...
def analyze_trades(df, short_window, long_window):
    ...
    df['Signal Shifted'] = df['Signal'].shift(1)
    df['Trade'] = np.where(df['Signal'] != df['Signal Shifted'], df['Signal'], np.nan)
    ...
    for i in range(len(df)-1):
        entry = df.iloc[i]
        exit = df.iloc[i+1]
        profit = (exit['Close'] - entry['Close']) / entry['Close'] if entry['Signal'] == 'Buy' else (entry['Close'] - exit['Close']) / entry['Close']
        win = 1 if profit > 0 else 0
        trades.append({'Trade': entry['Signal'], 'Profit': profit, 'Win': win})
    ...
    if not screened_df.empty:
        if summary_type == 'Detailed':
            ...
            def highlight_cross(x):
                color = 'green' if float(x.strip('%')) > 0 else 'red'
                return f'color: {color}'

            styled_df = screened_df.style.applymap(highlight_cross, subset=['% Crossed By'])
            st.dataframe(styled_df, use_container_width=True)
            st.download_button("Download CSV", screened_df.to_csv(index=False), "screened_stocks.csv")
        else:
            st.subheader("Summary Results")
            st.dataframe(screened_df, use_container_width=True)
            st.download_button("Download CSV", screened_df.to_csv(index=False), "summary_screened_stocks.csv")
    else:

...
# Install streamlit_autorefresh if not already installed
...
def analyze_trades(df, short_window, long_window):
    """
    Simulate buy/sell trades over the historical data and calculate win rate.
    Includes the date when each trade signal is generated.
    """
    df = calculate_moving_averages(df, short_window, long_window)
    df['Signal'] = np.where(df['SMA_short'] > df['SMA_long'], 'Buy', 'Sell')
    df['Signal_Shifted'] = df['Signal'].shift(1)
...
def screen_stocks(symbols, filter_param='Volume', threshold=0, short_window=9, long_window=21, 
    ...
    """
    Screen stocks based on SMA crossover and VCP pattern, calculate win rate.
    Returns both detailed and live calls dataframes.
    """
    data = {}
    summary_data = []
    live_calls = []

    current_week_start = datetime.now() - timedelta(days=datetime.now().weekday())  # Monday
    ...
    for idx, symbol in enumerate(symbols):
        ...
        try:
            ...
            # 3. Fetch recent data for current signals
            recent_data = fetch_data(symbol, period="1mo", interval="1d")
            if recent_data.empty:
                st.warning(f"No recent data for {symbol}. Skipping.")
                continue

            # 4. Calculate moving averages and volatility
            recent_data = calculate_moving_averages(recent_data, short_window, long_window)
            recent_data = calculate_relative_volatility(recent_data)

            # 5. Generate current buy/sell signal
            current_signal_data = detect_buy_sell_signals(recent_data)
            if current_signal_data is None:
                st.warning(f"Unable to generate signal for {symbol}. Skipping.")
                continue

            current_signal, cross_percentage = current_signal_data

            # 6. Identify if the signal was generated this week
            signal_date = recent_data.iloc[-1].name
            if current_week_start.date() <= signal_date.date() <= current_week_end.date():
                live_calls.append({
                    'Symbol': symbol,
    ...
    if summary_type == 'Detailed':
        df = pd.DataFrame.from_dict(data, orient='index')
        return df, pd.DataFrame(live_calls)
    else:
        summary_df = pd.DataFrame(summary_data)
        return summary_df, pd.DataFrame(live_calls)

# Auto-refresh every 5 minutes (300 seconds)
AUTO_REFRESH_INTERVAL = 300  # in seconds

st_autorefresh(interval=AUTO_REFRESH_INTERVAL * 1000, key="refresh")

# Main Execution
# Automatically run screening on app load and refresh
with st.spinner("Screening stocks... This may take a while..."):
    screened_df, live_calls_df = screen_stocks(
        nifty500_symbols,
        filter_param=filter_param,
        threshold=threshold,
        short_window=short_window,
...
# Display Live Calls
st.subheader("🔴 Live Calls (This Week)")
if not live_calls_df.empty:
    live_calls_df = live_calls_df[['Symbol', 'Date', 'Signal', '% Crossed By', 'Win Rate (%)', 'Risk-Reward']]
    st.dataframe(live_calls_df, use_container_width=True)
    st.download_button(
        "Download Live Calls CSV",
        live_calls_df.to_csv(index=False).encode('utf-8'),
        "live_calls.csv",
        mime='text/csv'
    )
else:
    st.warning("No live calls generated this week.")

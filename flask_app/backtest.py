...
# def calculate_percentage_change(df, period_days=10):
#     """
#     Calculate the percentage change in stock price over the past period_days (e.g., 2 weeks = 10 trading days).
#     """
#     if len(df) >= period_days:
#         recent_close = df['Close'].iloc[-1]
#         past_close = df['Close'].iloc[-period_days]
#         return (recent_close - past_close) / past_close * 100
#     else:
#         return np.nan

# def backtest(stock, initial_investment):
#     try:
#         # Calculate the date range (1 year of data to calculate SMA but focus on the past 2 weeks for results)
...
#         # Calculate the percentage change in the past 2 weeks (approximately 10 trading days)
#         percentage_change_2_weeks = calculate_percentage_change(df, period_days=10)

#         # Use the most recent data for the signal
#         latest_data = df.iloc[-1]
#         cross_percentage = (latest_data['SMA9'] - latest_data['SMA21']) / latest_data['SMA21'] * 100
        
#         # Determine the signal based on SMA cross
#         signal = 'Buy' if cross_percentage > 0 else 'Sell'

#         # Calculate final portfolio value based on the latest signal
#         final_value = initial_investment * (1 + cross_percentage / 100)
#         profit_loss = final_value - initial_investment
#         percentage_return = (profit_loss / initial_investment) * 100
...
#             '2-Week Change (%)': percentage_change_2_weeks
#         }
#     except Exception as e:
#         print(f"Error processing {stock}: {e}")
#         return None
